...
const MedFindComponents = {
  API_BASE: (location.hostname==='localhost'||location.hostname==='127.0.0.1')?'http://127.0.0.1:8000/api/v1':'https://medfind-backend-CHANGE-THIS.a.run.app/api/v1',
  ...
  injectNavbar() {
    ...
    // Skip navbar on dashboard/sidebar pages to avoid double navigation
    if (document.querySelector('.sidebar') || document.body.dataset.noNavbar === 'true') return;
  ...
  injectFooter() {
    ...
    // Skip footer on dashboard/sidebar pages
    if (document.querySelector('.sidebar') || document.body.dataset.noNavbar === 'true') return;
    const base = this.getBasePath();
  ...
  getCurrentUser() {
    try {
      ...
      // Normalise name — support full_name, name, firstName+lastName
      const computed = u.full_name || u.name
        || ((u.firstName||'') + (u.lastName ? ' '+u.lastName : '')).trim()
        || u.email?.split('@')[0] || 'User';
      return { ...u, name: computed };
    } catch { return null; }
  },
...
(function MedFindDevSignature() {
  ...
  function injectBar() {
    ...
    // Console watermark
    console.log(
      "%cMedFind — Built by Ahsanul Yamin Babor",
      "color:#fff;background:linear-gradient(135deg,#4f46e5,#7c3aed);font-size:13px;font-weight:800;padding:6px 16px;border-radius:6px;letter-spacing:-.2px"
    );
  }
