...
// Auto-detect API and WS base from window config or current hostname
const API_BASE = (window.MEDFIND_CONFIG && window.MEDFIND_CONFIG.API_BASE)
  || window.API_BASE
  || ((location.hostname === 'localhost' || location.hostname === '127.0.0.1')
      ? 'http://127.0.0.1:8000/api/v1'
      : location.origin + '/api/v1');

// WS uses same host/port as HTTP — just swap protocol
...
class MedFindCall {
  ...
  _log(...args) {
    if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
      console.log('%c[MedFind WebRTC]', 'color:#14b8a6;font-weight:bold', ...args);
    }
  }
}
